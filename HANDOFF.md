...
### CI 가 실패를 성공이라고 하던 문제

**빌드가 깨졌는데 워크플로가 통과로 넘어간 적이 있다.** `scripts/ci-run.sh` 에서
`if 명령; then ... fi` 뒤의 `$?` 를 읽어 `exit` 했는데, 조건이 거짓이고 `else` 가 없으면
`if` 문 자체는 0 을 돌려준다. 그래서 어떤 실패도 0 으로 나갔다.

지금은 상태를 `명령 || code=$?` 로 바로 받는다. **이 자리를 다시 건드리지 말 것.**

빌드가 진짜 됐는지 의심스러우면 `screenshots` 가지의 커밋이 방금 올린 커밋인지 본다.
빌드가 깨지면 화면이 안 찍히므로 가지가 옛날 커밋에 머문다.

실패하면 주석에 컴파일 오류 줄이 담긴다. 로그인 없이 이렇게 읽는다.
...
- 성적 기록을 적고 고치고 지운다. 과목별로 나눠 보고 등급 흐름을 막대로 본다
- 수능 시간표대로 시간을 잰다
- **등급컷으로 백분위와 표준점수를 예상한다.** 계산은 `MockScoring.swift` 에 있고
  사이트(`_source.html` 7004~7120줄)와 같은 값이 나와야 한다. 단조 3차 보간과
  z 값 회귀를 그대로 옮겼다. 예상값에는 화면에서 "예상" 이라고 붙인다
- 등급컷 배열에는 **빈 칸이 섞인다.** 사이트가 아는 등급컷만 적게 해서 나머지가 null 이다.
  `[Int]` 로 받으면 통째로 해석에 실패해 예상값이 사라진다. `[Int?]` 로 받는다
- 스피드 연산과 랭킹. **점수 공식을 바꾸면 안 된다.** 서버가 같은 식으로 검산해서
  정답 수로 낼 수 있는 최대를 넘으면 위조로 보고 거절한다

### 3단계 — 관리자 (됐다)

- 계정 화면에서 관리자로 들어간다. 관리자가 아니면 그 줄 자체가 없다
- 현황, 활동 기록, 회원. **회원 칸은 root 에게만 보인다.**
  활동 기록에서도 회원 관리 항목을 걸러낸다. root 라는 말은 어디에도 안 쓴다
- 시험 목록에서 출제와 수정, 제출 보기와 채점을 한다.
  남이 낸 시험은 root 만 고친다. 서버 정책도 같아서 앱이 잘못 불러도 거절당한다

**수식 입력기는 이렇게 정했다.** MathLive 는 웹 부품이라 옮길 수 없다.
웹뷰로 감싸면 앱 안에 웹을 다시 들이게 되고 인터넷이 끊기면 출제가 막힌다.
그래서 LaTeX 를 직접 적고 그린 모양을 바로 보여주는 쪽으로 갔다 (`MathInput.swift`).
자주 쓰는 기호는 단추로 넣고, 네모(□) 자리에 다음 값이 들어간다.
그리는 것은 SwiftMath 라 출제하며 보는 모양이 학생이 볼 모양과 같다.

### 남은 것

- **글번호로 여는 길.** `matches(no:)` 는 만들어뒀지만 아직 아무도 부르지 않는다.
  카톡으로 받은 링크를 앱이 열려면 유니버설 링크가 필요하고 애플 개발자 계정이 있어야 한다
- **로그인한 화면을 아직 못 봤다.** 위의 "아직 확인 못 한 것" 을 볼 것
- **서명과 실기기, 앱스토어 심사.** 전부 애플 개발자 계정이 있어야 한다
